...
class ApproximateQAgent(PacmanQAgent):
    ...
    def update(self, state, action, nextState, reward):
        ...
        """
        "*** YOUR CODE HERE ***"
        nextLegalActioins = self.getLegalActions(nextState)
        if len(nextLegalActioins) == 0:
            diff = reward - self.getQValue(state, action)
        else :
            diff = float('-inf')
            for nextAction in nextLegalActioins:
                diff = max(diff, reward + self.discount * self.getQValue(nextState, nextAction) - self.getQValue(state, action))
